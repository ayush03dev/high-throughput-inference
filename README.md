...
- **Rate-limit validation uses `admitted_at`, with a small tolerance for clock skew.** Both Lua scripts guarantee, by construction, that the count of ledger entries newer than `now - window` is at most the configured limit at the instant of every admission (verified directly against Redis, independent of load). `scenarios/validate.py` measures the same moment externally via each request's `admitted_at` (set in `markInFlight` immediately after `tryAcquire` succeeds). Measuring on `completed_at` instead used to inflate the sliding window by tens of seconds of queueing under saturation (~0.1% apparent overshoot, enough to flake Scenario 1 as `FAIL` on a strict cap); switching to `admitted_at` dropped that to ~11–17 requests (~0.02–0.03%) from JVM/Redis clock skew alone. Validate allows `RATE_LIMIT_TOLERANCE = 1.001` (0.1%) so the check is stable across machines without papering over real enforcement failures.
